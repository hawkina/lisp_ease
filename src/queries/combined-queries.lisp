...
(defun get-poses-from-event ()
  (let ((episode-instance)
    ...
    (progn
      ...
      (setq event-inst (string (cut:var-value '|?EventInst| (car (event-type "EventInst" "knowrob:'GraspingSomething'")))))
      (setq temp-list (u-occurs episode-instance event-inst "Start" "End"))
      (setq start (string (cut:var-value '|?Start| (car temp-list))))
      (setq end (string (cut:var-value '|?End| (car temp-list))))
      (setq obj-acted-on-inst (string (cut:var-value '|?ObjActedOnInst| (car (rdf-has event-inst "knowrob:'objectActedOn'" "ObjActedOnInst")))))
      (setq hand-inst (string (cut:var-value '|?HandInst| (car (performed-by event-inst "HandInst")))))
      (setq hand-inst-short (string (cut:var-value '|?HandInstShortName| (car (iri-xml-namespace hand-inst  "HandInstShortName")))))
      (setq hand-type (string (cut:var-value '|?HandType| (car (obj-type "HandInst" "HandType")))))
      (iri-xml-namespace hand-type "HandTypeName")
      (setq obj-short (string (cut:var-value '|?ObjShortName| (car (iri-xml-namespace obj-acted-on-inst  "ObjShortName")))))
      (setq camera-inst (string (cut:var-value '|?CameraInst| (car (obj-type "CameraInst" "knowrob:'CharacterCamera'")))))
      (setq camera-short (string (cut:var-value '|?CameraShortName| (car (iri-xml-namespace camera-inst "CameraShortName")))))
